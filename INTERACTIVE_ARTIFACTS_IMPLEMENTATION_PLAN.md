...
async def resume_chat(self, chat_id: str, enterprise_id: str):
...
**Key Insights:**
- **One document per enterprise** (application-level graph)
...
async def handle_websocket(...):
    ...
    # NEW: Validate workflow dependencies BEFORE starting/resuming
    dependencies_met, error_details = await validate_workflow_dependencies(
...
**Why This Works**:
- ✅ Persistent dependency graph per enterprise
...
export const ChatUIProvider = ({ children }) => {
  ...
  const navigateToWorkflow = async (targetWorkflow, context = {}) => {
    ...
    navigate(`/enterprise/${enterpriseId}/${targetWorkflow}`);
  };
...
- [ ] Add index on `enterprise_id` for fast graph lookups
...
- [ ] Add GET `/api/workflows/{enterprise_id}/available` endpoint
- [ ] Add POST `/api/workflows/{enterprise_id}/dependencies` endpoint (for manual overrides)
