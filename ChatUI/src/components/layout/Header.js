    ...
    `}>
      ...
      <div className="relative h-14 md:h-16 flex items-center justify-between px-4 md:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-3 md:gap-4">
          ...
          <div className="hidden md:flex items-center gap-2 text-xs text-cyan-300/80">
            <button onClick={handleMyAppsClick} className="flex items-center gap-1 hover:text-cyan-200 transition-colors">
            ...
            <span className="oxanium text-cyan-100 font-medium">
        ...
        <div className="flex items-center gap-1.5 md:gap-2.5">
          ...
          <div className="relative" ref={dropdownRef}>
            <button onClick={toggleProfileDropdown} className="flex items-center gap-2 p-2 rounded-lg hover:bg-white/10 transition-colors" title="Command Profile">
              <div className="relative">
                <div className="w-7 h-7 rounded-full overflow-hidden border border-cyan-400/30">
              ...
              <div className="hidden lg:block text-left">
                <div className="text-cyan-200 text-xs font-medium oxanium">{currentUser.firstName || 'Commander'}</div>
              </div>
              <svg className="w-3 h-3 text-cyan-400/60" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
              </svg>
            </button>
            {isProfileDropdownOpen && (
              <div className="absolute right-0 top-full mt-2 w-64 bg-slate-900/95 border border-cyan-400/40 rounded-2xl backdrop-blur-xl overflow-hidden z-50">
                <div className="relative p-4 border-b border-cyan-400/20">
                  <div className="flex items-center space-x-3">
                    <div className="relative">
                      <div className="w-12 h-12 rounded-full overflow-hidden border-2 border-cyan-400/40">
                        {currentUser.userPhoto ? (
                          <img src={currentUser.userPhoto} alt="User" className="w-full h-full object-cover" />
                        ) : (
                          <img src="/profile.svg" alt="profileicon" className="w-full h-full object-cover" />
                ...
                <div className="relative p-2">
                  <button className="w-full px-3 py-2.5 text-left text-cyan-100 hover:bg-cyan-400/10 rounded-xl transition-colors flex items-center gap-3" onClick={() => console.log('Navigate to profile')}>
                  ...
                  <button className="w-full px-3 py-2.5 text-left text-cyan-100 hover:bg-cyan-400/10 rounded-xl transition-colors flex items-center gap-3" onClick={() => console.log('Navigate to preferences')}>
                    <svg className="w-4 h-4 text-cyan-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
                  ...
                  <div className="border-t border-cyan-400/20 mt-2 pt-2">
                    <button onClick={() => console.log('Logout')} className="w-full px-3 py-2.5 text-left text-red-400 hover:bg-red-500/10 rounded-xl transition-colors flex items-center gap-3">
          ...
          <button onClick={toggleNotificationDropdown} className="relative p-2 rounded-lg hover:bg-white/10 transition-colors" title="Mission Alerts">
            <svg className="w-6 h-6 text-cyan-300" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.8}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M14.857 17.082a23.848 23.848 0 005.454-1.31A8.967 8.967 0 0118 9.75v-.7V9A6 6 0 006 9v.75a8.967 8.967 0 01-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 01-5.714 0m5.714 0a3 3 0 11-5.714 0" />
            </svg>
            {notificationCount > 0 && (
              <div className="absolute -top-0.5 -right-0.5">
                <div className="w-4 h-4 bg-red-500 rounded-full flex items-center justify-center border border-slate-900/60">
          ...
          {/* Discover */}
          <button onClick={handleDiscoverClick} className="hidden md:flex items-center gap-2 px-3 py-2 rounded-lg bg-white/5 border border-cyan-400/30 text-cyan-100 oxanium hover:bg-white/10 transition-colors text-sm" title="Discover New Features">
