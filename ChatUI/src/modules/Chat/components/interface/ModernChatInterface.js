...
// This component is defined *outside* ModernChatInterface to prevent re-creation on every render.
// It's memoized to avoid re-rendering if its props (agentUI, onAction) haven't changed.
const MemoizedAgentUIRenderer = React.memo(({ agentUI, onAction }) => {
  const { 
    component: AgentUIRendererComponent, 
    loading: componentLoading, 
    error: componentError 
  } = useDynamicComponent('AgentUIRenderer', 'inline-component');

  if (componentLoading) {
    return <div className="text-cyan-400 text-sm">Loading UI component...</div>;
  }

  if (componentError) {
    return (
      <div className="text-red-400 text-sm">
        Failed to load UI component: {componentError.message}
      </div>
    );
  }

  if (!AgentUIRendererComponent) {
    // This is not an error, the component might just not be available yet.
    // Or the dynamic import resolved to null/undefined.
    return null;
  }
          ...
          {messages?.map((chat, index) => {
            ...
            return (
              <div key={index}>
                ...
                {chat.agentUI && (
                  <MemoizedAgentUIRenderer 
                    agentUI={chat.agentUI}
