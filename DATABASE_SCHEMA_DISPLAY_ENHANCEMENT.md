...
- See which collections are enterprise-specific
  ...
  "collections": [
    {
      ...
      "sample_doc_keys": ["_id", "user_id", "email", "enterprise_id", ...]
    }
  ]
}
```
...
4. Detect enterprise-specific flag ("[Enterprise-specific]")
...
2. **Collections Grid**
   ...
   - Each collection card shows:
     ...
     * Enterprise-specific badge (if applicable)
...
3. **Schema Generation** (`_get_database_schema_async()`, line 239):
   ...
   - Identify enterprise-specific collections (have `enterprise_id` field)
...
3. **Transparency**: Know which collections are enterprise-scoped
...
**Agent Context**: WorkflowArchitectAgent sees `user_id`, `enterprise_id`, etc. and creates appropriate database variables:
```json
{
  "name": "user_profile",
  "type": "database",
...
┌─ Events [Enterprise] ────────┐  ┌─ Metrics [Enterprise] ───────┐
│ Fields (12)                  │  │ Fields (9)                   │
...
- `is_enterprise` (bool): Has enterprise_id field
...
"""
    ...
    assert result["collections"][0]["is_enterprise"] == True
```
...
✅ Enterprise-specific collections marked with badge  
