...
- **`selected_pattern`**
  ...
  - *How to Obtain (Pattern-Aware)*:
    1. Scan `InterviewTranscript` for structural keywords:
    ...
    4. Emit the integer Pattern ID.
- **`pattern_name`**
  - *Semantic Upstream Reference*: `selected_pattern`
  - *Rule/Taxonomy*: Lookup from `PATTERN_DISPLAY_NAME_BY_ID` in `pattern_guidance.md`.
  - *How to Obtain (Pattern-Aware)*:
    1. Use the `selected_pattern` integer.
    2. Return the corresponding display name from `PATTERN_DISPLAY_NAME_BY_ID`.
...
### Upstream Dependencies
- **`pattern_name`** (from `PatternSelection`)
...
- **`pattern`**
  - *Semantic Upstream Reference*: `PatternSelection.pattern_name`
  - *Rule/Taxonomy*: Start array with `pattern_name`; append supplementary patterns if requirements demand.
  - *How to Obtain (Pattern-Aware)*:
    1. Initialize array with `[pattern_name]`.
...
- **`ui_requirements.ui_pattern`**
  - *Semantic Upstream Reference*: Interaction Depth
  - *Rule/Taxonomy*: Input → `"single_step"`; Review → `"two_step_confirmation"`.
  - *How to Obtain (Pattern-Aware)*:
    1. If user provides data and flow continues → `"single_step"`.
    2. If user must review and approve before proceeding → `"two_step_confirmation"`.
    3. **Pattern Variation**: Feedback Loop (3) Review stages use `"two_step_confirmation"`.
...
- **`agents[].human_interaction`**
  ...
  - *How to Obtain (Pattern-Aware)*:
    1. If no UI requirement bound to this agent → `none`.
    2. If `ui_pattern` is `single_step` → `context`.
    3. If `ui_pattern` is `two_step_confirmation` → `approval`.
    4. If multi-step iteration → `feedback`.
    5. **Pattern Variation**: Feedback Loop (3) Review agent uses `approval` or `feedback`.
...
- **`orchestration_pattern`**
  ...
  - *Rule/Taxonomy*: Copy `pattern_name`.
  - *How to Obtain (Pattern-Aware)*:
    1. Copy `PatternSelection.pattern_name`.
