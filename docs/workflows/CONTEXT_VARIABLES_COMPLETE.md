...
1. **Minimal Base Context**
   - Seeds `enterprise_id`, `user_id`, `workflow_name`
...
async def _resolve_data_reference(runtime, definition):
    ...
    Important: When multiple documents match the query (e.g., duplicate enterprise_id entries),
...
```json
{
  "agents": {
    "MyAgent": {
      "variables": ["enterprise_id", "feature_x_enabled", "user_preferences"]
    }
  }
}
```
...
**Problem**: Multiple documents with the same query criteria (e.g., same `enterprise_id`) can exist in the database.
...
- Multiple workflow runs creating concepts for same enterprise_id
...
{
  "indexes": [
    {"keys": [["enterprise_id", 1]], "unique": true}
  ]
}
```
