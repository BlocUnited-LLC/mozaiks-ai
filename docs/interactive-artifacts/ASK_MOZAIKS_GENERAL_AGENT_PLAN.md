...
- **Transport + FastAPI** (`shared_app.py`)
  - `/ws/{workflow_name}/{enterprise_id}/{chat_id}/{user_id}` wires clients into AG2 workflows and now carries `ws_id` metadata for switching.
  - `/api/sessions/list/{enterprise_id}/{user_id}` lets the UI fetch tabs to render "Generator", "Investor", etc.
...
- **Context Awareness Today**
  - The runtime knows enterprise_id, user_id, workflow_name, and existing chat_ids, but it does not derive a higher-level "user journey" summary.
...
5. The agent already knows:
   ...
   - Enterprise/user metadata for tenancy enforcement.
...
2. **Context Package Builder**
   ...
   - Optional future hook: generator-provided KB stub stored per enterprise/workflow.
3. **Transport Hooks**
...
5. **Observability**
   - Emit structured logs and metrics (`general_agent_call_duration`, `general_agent_tokens_used`) for MozaiksStream billing alignment.
...
- `enterprise_id`, `user_id`, `ws_id` (for tenancy + logging).
...
3. **Phase 2 – Generator Knowledge Pack (Optional)**
   ...
   - Runtime loads the pack per enterprise_id and merges it into the context package.
...
- **LLM Configuration**
  ...
  - Provide switches for enterprise-specific providers if needed.
...
3. **Observability**
   ...
   - Validate MozaiksStream token accounting hooks receive per-call usage data.
...
2. **Conversation Persistence** – Do we keep a single Ask Mozaiks chat per ws_id, or persist per enterprise/user for long-term knowledge?
3. **Knowledge Pack Ownership** – Should generator outputs live in `workflows/<app>/metadata/ask_mozaiks.json`, or a shared `knowledge-packs/` directory keyed by enterprise_id?
4. **Actionability** – Can Ask Mozaiks trigger workflow events (e.g., auto-start Investor) or is it strictly informational for MVP?
5. **Authentication** – When exposing this as an overlay for third-party apps, do we need additional scopes/keys beyond enterprise_id & user_id?
